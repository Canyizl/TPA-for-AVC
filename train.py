...
parser.add_argument("--alg", type=str, nargs="?",
                    default="tpamaddpg", help="Please enter the alg name.")
...
log_name = "-".join([argv.env, net_topology, argv.mode,
                    argv.alg, argv.voltage_barrier_type, str(argv.seed), argv.alias])
...
if argv.alg == "tpamaddpg" or argv.alg == "tpamatd3":
    alg_config_dict["tpa"] =  True
else:
    alg_config_dict["tpa"] =  False
...
for i in range(args.train_episodes_num):
    stat = {}
    train.run(stat, i, save_path, log_name)
