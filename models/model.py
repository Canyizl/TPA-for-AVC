...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.Transition = namedtuple('Transition', ('state','state_time', 'month', 'action', 'log_prob_a', 'value', 'next_value',
                                     'reward', 'cost', 'next_state','next_state_time', 'next_month', 'done', 'last_step', 'action_avail', 'last_hid', 'hid'))
    ...
    def policy(self, obs, obs_time, month, schedule=None, last_act=None, last_hid=None, info={}, stat={}):
        # obs_shape = (b, n, o)
        ...
        if self.args.shared_params:
            ...
            means, log_stds, hiddens = agent_policy(obs, obs_time, month, last_hid)
    ...
    def train_process(self, stat, trainer):
        ...
        comb_state, global_state, epi_year = trainer.env.reset()
        state = comb_state["agents_obs"]
        state_time = comb_state["time_obs"]
        month = comb_state["month"]

        #epi_year_ = th.tensor(epi_year).to(self.device).contiguous().view(-1,1)
        ...
        for t in range(self.args.max_steps):
            ...
            with th.no_grad():
                action, action_pol, log_prob_a, _, hid = self.get_actions(state_, state_time_, month, status='train', exploration=True, actions_avail=th.tensor(
            ...
            next_comb_state = trainer.env.get_obs()
            ...
            next_state_time_ = prep_obs(next_state_time).to(self.device).contiguous().view(
                self.n_ ,self.tp, self.time_dims)
            with th.no_grad():
                _, next_action_pol, _, _, _ = self.get_actions(next_state_, next_state_time_, next_month, status='train', exploration=True, actions_avail=th.tensor(
    ...
    def evaluation(self, stat, trainer, test_season='June'):
        ...
        if test_season == "June":
            ...
            NotImplementedError()
        #test_data = np.array(test_data) % 365 + 365 * 2
        ...
        with th.no_grad():
            for _ in range(num_eval_episodes):
                ...
                #    test_data[_], 23, 2)
                comb_state, global_state, epi_year = trainer.env.manual_reset(test_data[_], 23, 2) 
                state = comb_state["agents_obs"]
                state_time = comb_state["time_obs"]
                month = comb_state["month"]
                # epi_year_ = th.tensor(epi_year).to(self.device).contiguous().view(-1,1)
                ...
                for t in range(self.args.max_eval_steps):
                    ...
                    state_time_ = prep_obs(state_time).to(self.device).contiguous().view(
                        self.n_ ,self.tp, self.time_dims)
                    action, _, _, _, hid = self.get_actions(state_, state_time_, month, status='test', exploration=False, actions_avail=th.tensor(
                        trainer.env.get_avail_actions()), target=False, last_hid=last_hid)
    ...
    def unpack_data(self, batch):
        ...
        state_time = prep_obs(list(zip(batch.state_time))).to(self.device)
        month = th.tensor(np.concatenate(batch.month, axis=0),
                           dtype=th.float).to(self.device)
        next_state = prep_obs(list(zip(batch.next_state))).to(self.device)
        next_state_time = prep_obs(list(zip(batch.next_state_time))).to(self.device)
        next_month = th.tensor(np.concatenate(batch.next_month, axis=0),
                           dtype=th.float).to(self.device)
        ...
        return (state, state_time, month, action, log_prob_a, value, next_value, reward, cost, next_state, next_state_time, next_month, done, last_step, action_avail, last_hid, hid)
