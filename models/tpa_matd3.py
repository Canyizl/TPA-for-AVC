...
class TPAMATD3(Model): 
    def __init__(self, args, target_net=None):
        super(TPAMATD3, self).__init__(args)
        # for constraint
        self.cs_num = self.n_
    ...
    def get_actions(self, state, state_time, month, status, exploration, actions_avail, target=False, last_hid=None):
        ...
        if self.args.continuous:
            means, log_stds, hiddens = self.policy(state, state_time, month, last_hid=last_hid) if not target else target_policy(state, state_time, month, last_hid=last_hid)
    ...
    def get_prop_loss(self, batch):
        ...
        '''
        #sparsity loss
        l1_mask = 1 - th.t(agent_policy.prototype_class_identity).to(self.device)
        l1 = (agent_policy.last_layer.weight * l1_mask).norm(p=1)
        loss = loss + 5e-4 * l1
        '''
        batch_loss = 0
