...
class MATD3(Model):
    ...
    def get_actions(self, state,state_time,state_month, status, exploration, actions_avail, target=False, last_hid=None):
        ...
        if self.args.continuous:
            means, log_stds, hiddens = self.policy(
                state,state_time,state_month, last_hid=last_hid) if not target else target_policy(state,state_time,state_month, last_hid=last_hid)
    ...
    def get_loss(self, batch):
        ...
        state,state_time,state_month, actions, old_log_prob_a, old_values, old_next_values, rewards, cost, next_state,next_state_time,next_state_month, done, last_step, actions_avail, last_hids, hids = self.unpack_data(
            batch)
        _, actions_pol, log_prob_a, action_out, _ = self.get_actions(
            state,state_time,state_month, status='train', exploration=False, actions_avail=actions_avail, target=False, last_hid=last_hids)
        ...
        if self.args.double_q:
            _, next_actions, _, _, _ = self.get_actions(
                next_state,next_state_time,next_state_month, status='train', exploration=True, actions_avail=actions_avail, target=False, last_hid=hids)
        else:
            _, next_actions, _, _, _ = self.get_actions(
                next_state,next_state_time,next_state_month, status='train', exploration=True, actions_avail=actions_avail, target=True, last_hid=hids)
