...
class VoltageControl(MultiAgentEnv):
    ...
    def __init__(self, kwargs):
        ...
        if self.args.season == 'all':
            ...
            #self.start_train_day_func = self._select_start_train_day
        ...
        comb_agents_obs, state, epi_year = self.reset()
        agents_obs = comb_agents_obs["agents_obs"]
    ...
    def reset(self, reset_time=True):
        ...
        while not solvable:
            ...
            # day means the all days select one day, so no month or week
        ...
        return self.get_obs(), None, None #self.get_state()

    def time_reset(self, reset_time=True):
        """reset the env
        """
        # reset the time step, cumulative rewards and obs history
        self.steps = 1
        ...
        while not solvable:
            ...
            time_length = 10
            # get one episode of data
            self.pv_histories,self.active_demand_histories,self.reactive_demand_histories = self._get_time_history(time_length)
            self._set_demand_and_pv()
            # random initialise action
            if self.args.reset_action:
                self.powergrid.sgen["q_mvar"] = self.get_action()
    ...
    def manual_reset(self, day, hour, interval):
        ...
        return self.get_obs(), None, None #self.get_state(), 2
    ...
    def get_24jieqi(self, year=2012):
        all_start_dayidx = datetime(2012,1,1)
        all_jieqi = list()
        all_jq_days = [(2,3),(2,18),(3,5),(3,20),(4,4),(4,19),(5,5),(5,20),(6,5),(6,21),(7,7),(7,23),(8,7),(8,22),(9,8),(9,22),(10,7),(10,23),(11,7),(11,22),(12,6),(12,22),(1,5),(1,19)]
        oneday_long = 24 * (60 // self.time_delta)
        self.oneday_long = oneday_long
        for jq_month,jq_day in all_jq_days:
            dayidx = (datetime(year,jq_month,jq_day) - all_start_dayidx).days
            jieqi_idx =  dayidx * 24 * (60 // self.time_delta)
            #all_jieqi_idx.append(jieqi_idx)
             
            st_idx = jieqi_idx
            end_idx = st_idx + oneday_long
            tp_pv = self.pv_data[st_idx:end_idx].values
            tp_active = self.active_demand_data[st_idx:end_idx].values
            tp_reactive = self.reactive_demand_data[st_idx:end_idx].values

            jieqi_pv = np.mean(tp_pv, axis=-1)
            jieqi_active = np.mean(tp_active, axis=-1)
            jieqi_react = np.mean(tp_reactive, axis=-1)
            jieqi = np.array([jieqi_pv,jieqi_active,jieqi_react]).reshape(-1,3)
            all_jieqi.append(jieqi)
        all_jieqi = np.array(all_jieqi)
        return all_jieqi
    ...
    def get_obs(self):
        ...
        '''
        Here To get the Time Transformer inputs (pv, active, reactive)
        Timeperiod: tp= 100
        '''
        tp = self.tp
        end_idx = self.start_idx + self.steps - 1
        st_idx = end_idx - tp
        tp_pv = self.pv_data[st_idx:end_idx].values
        tp_active = self.active_demand_data[st_idx:end_idx].values
        tp_reactive = self.reactive_demand_data[st_idx:end_idx].values
        ...
        if self.args.mode == "distributed":
            ...
            for i in range(len(self.powergrid.sgen)):
                ...
                date_len = len(pv_flag)
                
                load_index = self.powergrid.load.sort_index().loc[self.powergrid.load["bus"]==sgen_bus].index
                tp_pv_np = np.array(tp_pv[:, i]).reshape(-1,1)
                tp_act_np = np.array(tp_active[:, load_index])
                tp_rea_np = np.array(tp_reactive[:, load_index])
                obs_time = np.concatenate([tp_pv_np, tp_act_np, tp_rea_np], axis=-1)

                idx = self.start_idx + self.steps - 1
                month = np.array(self.pv_data.index[idx].month).reshape(-1,1)

                if "date" in self.state_space:
                    idx = self.start_idx + self.steps - 1
                    month = np.array(self.pv_data.index[idx].month)
                    obs_month = np.repeat(month, date_len)
                    obs += list(obs_month)
                ...
                if "demand" in self.state_space:
                    ...
                    #print("p_mw: ", copy_zone_buses.loc[:, "p_mw"].to_numpy().shape)
                    obs += list(copy_zone_buses.loc[:, "q_mvar"].to_numpy(copy=True))
                    #print("q_mvar: ", copy_zone_buses.loc[:, "q_mvar"].to_numpy().shape)
            ...
            time_obs = np.array(obs_time_zone_dict)
        elif self.args.mode == "decentralised":
            obs_len_list = list()
            zone_obs_list = list()
        ...
        if self.history > 1:
            ...
            for i, obs in enumerate(agents_obs):
                ...
                print("self.obs_history[i]", torch.tensor(self.obs_history[i]).shape)
            agents_obs = agents_obs_
        #print("agents_obs:", np.array(agents_obs).shape)
        ...
        #return agents_obs
        return comb_agents_obs
    
    def get_transformer_obs(self):
        pass
    ...
    def _select_start_train_day(self):
        """select start day (date) for an episode
        """
        pv_data = self.pv_data
        pv_days = (pv_data.index[-1] - pv_data.index[0]).days - 365
    ...
    def _select_start_day(self):
        ...
        # episode_days = 1
        tp_margin = 1
        return np.random.choice(pv_days - episode_days - tp_margin) + tp_margin
    ...
    def _get_time_history(self, history):
        ...
        nr_intervals = episode_length + history
        true_start = start - nr_intervals
        if true_start < 0 :
            true_start = 0
        episode_pv_history = self.pv_data[true_start : start+1].values
        episode_act_history = self.active_demand_data[true_start : start+1].values
        episode_react_history = self.reactive_demand_data[true_start : start+1].values
        return episode_pv_history, episode_act_history, episode_react_history
